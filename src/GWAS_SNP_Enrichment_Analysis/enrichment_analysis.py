...
"""
Function: This script was used to do enrichment analysis for GWAS SNPs,
...
def get_tissue_and_class_from_snp_input(snp_input_file):
    class_name = os.path.basename(snp_input_file).split("_")[0].lower()
    tissue_name = "_".join(os.path.basename(snp_input_file).split("_")[1:-1]).lower()
...
def get_tissue_and_class_from_snp_m6a(snp_m6a_file):
    tissue_name = "_".join(os.path.basename(snp_m6a_file).split("_")[1:]).split(".bed")[0].lower()
    class_name = os.path.basename(snp_m6a_file).split("_")[0].lower()
